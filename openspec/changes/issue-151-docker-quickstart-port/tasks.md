...
- [x] 1.1a Extend the test to assert the `app` service sets `HOSTNAME=0.0.0.0` via `environment:` (so a `HOSTNAME` from the operator's `env_file` cannot bind the server to an unreachable address)
...
- [x] 2.1 Add `environment:` with `PORT=3000` and `HOSTNAME=0.0.0.0` to the `app` service in `docker-compose.yml`
