...
import { CalendarEvent, CalendarFeed, CalendarView } from "@/types/calendar";
...
export const useViewStore = createStandardStore({
  ...
  persistOptions: {
    ...
    partialize: (state: ViewState & ViewActions) => ({
      view: state.view,
    ...
    onRehydrateStorage:
      () => (state: (ViewState & ViewActions) | undefined) => {
        if (state) {
  ...
  // Custom clear that only resets selectedEventId
  customClear: (set) => {
    set((state: ViewState) => ({
      ...state,
      selectedEventId: undefined,
...
export const useCalendarUIStore = createStandardStore({
  ...
  persistOptions: {
    ...
    onRehydrateStorage: () => (state: (UIState & UIActions) | undefined) => {
      if (state) {
...
// Main Calendar Store interfaces
...
export const useCalendarStore = createStandardStore({
  name: "calendar-store",
  initialState: {
    // Initial state
    feeds: [],
    ...
    selectedView: "week" as CalendarView,
    ...
    ({
              ...
              occurrences.forEach((date) => {
                ...
                const hasModifiedInstance = events.some(
                  (e) =>
                    ...
                    newDate(e.start).toDateString() ===
                      instanceDate.toDateString()
                );
      ...
      addFeed: async (name, url, type, color) => {
        ...
        try {
          ...
          if (url) {
            await (get() as CalendarStoreState & CalendarStoreActions).syncFeed(
              id
            );
          }
        } catch (error) {
      ...
      syncFeed: async (id) => {
        ...
        try {
          ...
          await (get() as CalendarStoreState & CalendarStoreActions).updateFeed(
            id,
            {
      ...
      syncAllFeeds: async () => {
        const { feeds } = get();
        const store = get() as CalendarStoreState & CalendarStoreActions;
        for (const feed of feeds) {
          if (feed.enabled) {
            await store.syncFeed(feed.id);
          }
        }
      },
      ...
      getAllCalendarItems: (start: Date, end: Date) => {
        ...
        const store = get() as CalendarStoreState & CalendarStoreActions;
        const events = store.getExpandedEvents(start, end);
        const taskEvents = store.getTasksAsEvents(start, end);
        return [...events, ...taskEvents];
      },
    }) satisfies CalendarStoreActions,

  // Custom clear that resets data but preserves selected date/view
  customClear: (set) => {
    set((state: CalendarStoreState) => ({
      ...
      isLoading: false,
      error: undefined,
      // Keep selectedDate and selectedView as they're user preferences
    }));
  },
});
