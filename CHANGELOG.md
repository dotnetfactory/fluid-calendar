...
- Fixed CalDAV events created or updated in FluidCalendar appearing shifted by the server's UTC offset in other calendar clients (Thunderbird, Home Assistant); timed `DTSTART`/`DTEND` (and `RECURRENCE-ID`/`EXDATE`) are now serialized as unambiguous UTC date-times (`...Z`) instead of floating local time, so they no longer require the `TZ` environment-variable workaround (#135)
