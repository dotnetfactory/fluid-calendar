...
- Fixed the Docker quick-start being unreachable on `http://localhost:3000` when the operator's `.env` contained an unrelated `PORT` (e.g. `PORT=80`): `docker-compose.yml` forwarded the whole `.env` into the app container, so the app bound to that port while compose still published `3000:3000`. The `app` service now pins `PORT=3000`, so the published port always reaches a listening server; the README quick-start documents that `PORT` in `.env` does not change the published port (#151)
